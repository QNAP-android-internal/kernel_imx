...
/*
 * DRM driver for PIXPAPER 2.13" monochrome e-ink panel
 *
 * The panel is driven by an active matrix EPD controller (max 176 source
 * x 296 gate, B/W/Red). This driver targets the 122 (source/X) x 250
 * (gate/Y) black/white variant.
 *
 ...
 *
 * The X axis maps to the source outputs and the Y axis to the gate
 * outputs. The controller supports up to 176 source x 296 gate; this
 * 2.13" B/W panel uses 122 x 250.
 */
#define PIXPAPER_WIDTH    122
#define PIXPAPER_HEIGHT   250

/*
 * The controller addresses RAM in the X (source) direction by 8-pixel units
 * (one byte per address). Each gate line therefore needs
 * DIV_ROUND_UP(width, 8) bytes; the 6 unused bits of the last byte map to
 * non-existent sources and are ignored by the panel.
 */
#define PIXPAPER_WIDTH_BYTES   DIV_ROUND_UP(PIXPAPER_WIDTH, 8)

/*
 * The 2.13" panel active area is 23.71 mm x 48.55 mm.
...
/*
 * The panel provides no RGB-style timing parameters
 * (porches, sync widths, or a dot clock). Define a minimal
...
/* Panel controller command set. */
#define PIXPAPER_CMD_DRIVER_OUTPUT_CTRL      0x01
#define PIXPAPER_CMD_BOOSTER_SOFT_START_CTRL 0x0C
#define PIXPAPER_CMD_DEEP_SLEEP_MODE         0x10
#define PIXPAPER_CMD_DATA_ENTRY_MODE         0x11
#define PIXPAPER_CMD_SW_RESET                0x12
...
/*
 * Driver Output Control (0x01) third byte: GD = 0 (G0 is first gate),
 * SM = 0 (no odd/even gate split), TB = 0 (scan from top to bottom).
 * This is the controller power-on default.
 */
#define PIXPAPER_DRIVER_OUTPUT_GD_SM_TB         0x00

/*
 * Data Entry Mode (0x11): AM = 0 (counter advances in X), ID[1:0] = 11
 * (X increment, Y increment). RAM is therefore filled left-to-right,
 * top-to-bottom.
 */
#define PIXPAPER_DATA_ENTRY_X_INC_Y_INC         0x03

/*
 * Border Waveform Control (0x3C): A[7:6] = 00 (GS transition), A[2] = 1
 * (follow LUT), A[1:0] = 01 (LUT1). Standard border for a B/W full
 * refresh.
 */
#define PIXPAPER_BORDER_GS_LUT1                 0x05

/* Temperature Sensor Control (0x18): use the internal sensor. */
#define PIXPAPER_TEMP_SENSOR_INTERNAL           0x80

/*
 * Booster Soft Start Control (0x0C) is a four-byte command (phase 1/2/3
 * soft start plus duration). These are the controller power-on-reset values.
 */
#define PIXPAPER_SOFTSTART_PHASE1               0x8B
#define PIXPAPER_SOFTSTART_PHASE2               0x9C
#define PIXPAPER_SOFTSTART_PHASE3               0x96
#define PIXPAPER_SOFTSTART_DURATION             0x0F

/*
 * Display Update Control 2 (0x22) sequence value for a full refresh:
 * enable clock -> enable analog -> load temperature -> display with
 * DISPLAY Mode 1 -> disable analog -> disable OSC.
 */
#define PIXPAPER_UPDATE_SEQ_FULL                0xF7

/* Deep Sleep Mode (0x10): enter deep sleep mode 1. */
#define PIXPAPER_DEEP_SLEEP_MODE1               0x01
...
static const u8 pixpaper_init_driver_output[] = {
	...
	((PIXPAPER_HEIGHT - 1) >> 8) & 0x01,
	PIXPAPER_DRIVER_OUTPUT_GD_SM_TB,
};

static const u8 pixpaper_init_data_entry[] = {
	PIXPAPER_DATA_ENTRY_X_INC_Y_INC,
};

/*
 * Set RAM X start/end (0x44). The X address is in byte units, so the end
 * position is the number of bytes per line minus one.
 */
static const u8 pixpaper_init_ram_x_window[] = {
	PIXPAPER_RAM_START_ADDR,
	(PIXPAPER_WIDTH_BYTES - 1) & 0x1f,
};

/*
 * Set RAM Y start/end (0x45). The Y address is a 9-bit per-line value,
 * sent as low byte + high bit for both start and end.
 */
...
static const u8 pixpaper_init_softstart[] = {
	PIXPAPER_SOFTSTART_PHASE1,
	PIXPAPER_SOFTSTART_PHASE2,
	PIXPAPER_SOFTSTART_PHASE3,
	PIXPAPER_SOFTSTART_DURATION,
};

/* X address counter (0x4E) is a single byte. */
static const u8 pixpaper_init_ram_x_counter[] = {
	PIXPAPER_RAM_START_ADDR,
};

/* Y address counter (0x4F) is a 9-bit value: low byte + high bit. */
...
/*
 * Initialization sequence following the controller operation flow:
 * driver output, data entry mode, RAM window,
 * border, temperature sensor and booster soft start, then reset the RAM
 * address counters.
 */
static const struct pixpaper_init_seq pixpaper_init_seqs[] = {
	...
	{
		.cmd = PIXPAPER_CMD_DATA_ENTRY_MODE,
		.data = pixpaper_init_data_entry,
		.len = ARRAY_SIZE(pixpaper_init_data_entry),
	},
	{
					...
					struct pixpaper_error_ctx *err)
{
	...
	/* Y address counter (0x4F) is a low byte plus a high bit. */
				    ...
				    struct pixpaper_error_ctx *err)
{
	...
	pixpaper_send_data(panel, PIXPAPER_UPDATE_SEQ_FULL, err);
				    ...
				    u32 width, u32 src_pitch, u32 dst_pitch)
{
	...
	for (u32 y = 0; y < height; y++) {
		...
		for (u32 x = 0; x < width; x++) {
			/*
			 * Data Entry Mode is set to X increment, so RAM byte 0
			 * bit 7 maps to source S0 (panel left). Pack pixels in
			 * natural left-to-right order, MSB first.
			 */
			uint8_t r, g, b;
			u8 bit;
			u32 bit_pos = x % 8;
			...
			pixel = le32_to_cpu(src_pixels[x]);
			r = (pixel >> 16) & 0xFF;
			g = (pixel >> 8) & 0xFF;
			b = pixel & 0xFF;
			...
			/* RAM bit 1 = white pixel, 0 = black (command 0x24). */
...
static int pixpaper_panel_hw_init(struct pixpaper_panel *panel)
{
	 ...
	 * hardware reset, then a software reset (0x12) followed by a short
	 * settle delay. SW RESET restores all registers except deep sleep
	 * mode to their defaults and asserts BUSY while it runs.
	 */
	...
	pixpaper_send_cmd(panel, PIXPAPER_CMD_SW_RESET, &err);
	if (err.errno_code)
		goto init_fail;
	msleep(10);
	pixpaper_wait_for_panel(panel);
...
static struct drm_driver pixpaper_drm_driver = {
	...
	.fops = &pixpaper_fops,
	.name = "pixpaper-213m",
	.desc = "DRM driver for PIXPAPER 2.13\" monochrome e-ink panel",
	.major = 1,
	.minor = 0,
...
static void pixpaper_remove(struct spi_device *spi)
{
	...
	/*
	 * Put the controller into deep sleep before tearing down.
	 * Exiting deep sleep requires a hardware reset,
	 * which probe() performs the next time the driver binds.
	 */
	pixpaper_send_cmd(panel, PIXPAPER_CMD_DEEP_SLEEP_MODE, &err);
	pixpaper_send_data(panel, PIXPAPER_DEEP_SLEEP_MODE1, &err);

	drm_dev_unplug(&panel->drm);
	drm_atomic_helper_shutdown(&panel->drm);
}

static const struct spi_device_id pixpaper_ids[] = { { "pixpaper-213m", 0 }, {} };
MODULE_DEVICE_TABLE(spi, pixpaper_ids);

static const struct of_device_id pixpaper_dt_ids[] = {
	{ .compatible = "mayqueen,pixpaper-213m" },
	{}
};
...
MODULE_DESCRIPTION("DRM SPI driver for PIXPAPER 2.13\" monochrome e-ink panel");
MODULE_LICENSE("GPL");
